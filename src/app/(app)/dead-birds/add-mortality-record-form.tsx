  ...
  return (
    <Button type="submit" disabled={pending} className="bg-destructive text-destructive-foreground hover:bg-destructive/90">
      {pending ? (
        ...
        <>
          <PlusCircle className="mr-2 h-4 w-4" /> Log Dead Birds
        </>
      )}
    </Button>
  );
...
interface AddMortalityRecordFormProps {
    userName: string;
    isManager: boolean;
    assignedShed: string | null;
}

export function AddMortalityRecordForm({ userName, isManager, assignedShed }: AddMortalityRecordFormProps) {
      ...
      <form action={formAction} ref={formRef}>
        <CardHeader>
          <CardTitle className="font-headline flex items-center gap-2">
            <BirdIcon className="h-6 w-6 text-destructive" /> Record Dead Birds
          </CardTitle>
          <CardDescription>Log instances of bird mortality. The date is automatically set to today.</CardDescription>
        </CardHeader>
